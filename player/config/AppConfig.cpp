...
FilePath AppConfig::resourceDirectory() { return resourceDirectory_; }

void AppConfig::resourceDirectory(const FilePath &directory) {
  if (!FileSystem::exists(directory))
    throw PlayerRuntimeError{"AppConfig",
                             "Resource directory doesn't exist. Create or use "
                             "exsiting one in the player options app."};
...
FilePath AppConfig::publicKeyPath() { return configDirectory() / "id_rsa.pub"; }

FilePath AppConfig::privateKeyPath() { return configDirectory() / "id_rsa"; }
...
FilePath AppConfig::cachePath() { return configDirectory() / "cacheFile.xml"; }

FilePath AppConfig::statsCache() { return configDirectory() / "stats.sqlite"; }

FilePath AppConfig::additionalResourcesDirectory() {
  auto additionalResourcesDir = std::filesystem::path("/usr/share/xibo");
  if (!std::filesystem::exists(additionalResourcesDir)) {
    Log::error("[AppConfig] /usr/share/xibo does not exists. Using: {}",
               execDirectory().relative_path().string());
    return execDirectory();
  }
  return FilePath{additionalResourcesDir};
}
