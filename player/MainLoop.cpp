...
MainLoop::MainLoop(const std::string &name)
    : parentApp_(Gtk::Application::create(name)) {
  g_uiThreadId = std::this_thread::get_id();
}

std::thread::id MainLoop::uiThreadId() { return g_uiThreadId; }
...
void MainLoop::quit() { parentApp_->quit(); }

SignalStarted &MainLoop::started() { return started_; }
...
void MainLoop::setIdleAction(const IdleAction &action) {
  idleConnection_ =
      Glib::MainContext::get_default()->signal_idle().connect(action);
}
