...
template <> struct type_printer<Stats::RecordType> : public text_printer {};

template <> struct type_printer<DateTime> : public integer_printer {};

template <> struct statement_binder<Stats::RecordType> {
  std::time_t bind(sqlite3_stmt *stmt, int index,
                   const Stats::RecordType &value) {
    return statement_binder<std::string>().bind(
        stmt, index, Stats::recordTypeToString(value));
  }
};

template <> struct statement_binder<DateTime> {
...
template <> struct field_printer<Stats::RecordType> {
  std::string operator()(const Stats::RecordType &t) const {
    return Stats::recordTypeToString(t);
  }
};

template <> struct field_printer<DateTime> {
  std::time_t operator()(const DateTime &dt) const { return dt.timestamp(); }
};

template <> struct row_extractor<Stats::RecordType> {
  Stats::RecordType extract(const char *row_value) {
    if (auto type = Stats::recordTypeFromSting(row_value)) {
      return *type;
    }
    throw std::runtime_error("incorrect record type string (" +
...
template <> struct row_extractor<DateTime> {
  DateTime extract(const char *row_value) {
    std::time_t timestamp;
    auto [p, ec] =
        std::from_chars(row_value, row_value + strlen(row_value), timestamp);
    if (ec == std::errc()) {
      return DateTime::utcFromTimestamp(timestamp);
    }
    throw std::runtime_error("incorrect timestamp string (" +
                             std::string(row_value) + ")");
  }
...
inline auto initStorage(const FilePath &path) {
  ...
  return make_storage(
      path.string(),
      make_table("stats", make_column("id", &RecordDto::id, primary_key()),
...
struct DatabaseProvider::PrivateData {
  using Storage = decltype(initStorage(FilePath("")));
...
DatabaseProvider::DatabaseProvider()
    : data_(
          std::make_unique<PrivateData>(initStorage(AppConfig::statsCache()))) {
  try {
...
void DatabaseProvider::save(PlayingRecordDtoCollection &&records) {
  ...
  if (!result)
    throw Error{"database transaction failed"};
}
...
void DatabaseProvider::removeAll() { data_->db.remove_all<RecordDto>(); }
