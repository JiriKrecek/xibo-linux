...
void XmlFormatter::fillBase(const Record &record) {
  node_.put(Parsing::xmlAttr("scheduleid"),
            std::to_string(record.scheduleId()));
  node_.put(Parsing::xmlAttr("fromdt"),
            record.started().string("%Y-%m-%d %H:%M:%S"));
  node_.put(Parsing::xmlAttr("todt"),
            record.finished().string("%Y-%m-%d %H:%M:%S"));
...
void XmlFormatter::visit(const LayoutRecord &record) {
  ...
  node_.put(Parsing::xmlAttr("type"),
            Stats::recordTypeToString(RecordType::Layout));
...
void XmlFormatter::visit(const MediaRecord &record) {
  ...
  node_.put(Parsing::xmlAttr("type"),
            Stats::recordTypeToString(RecordType::Media));
