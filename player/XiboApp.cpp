...
XiboApp::XiboApp(const std::string &name)
  ...
  if (!FileSystem::exists(AppConfig::cmsSettingsPath()))
    throw PlayerRuntimeError{"XiboApp",
                             "Update CMS settings using player options app"};

  playerSettings_.logLevel().valueChanged().connect(
      [](const std::string &logLevel) { Log::setLevel(logLevel); });
...
std::unique_ptr<XmrManager> XiboApp::createXmrManager() {
  auto xmrChannel = XmrChannel::fromCmsSettings(
      cmsSettings_.address(), cmsSettings_.key(), cmsSettings_.displayId());
  auto manager = std::make_unique<XmrManager>(xmrChannel);

  manager->connect(playerSettings_.xmrNetworkAddress());
  playerSettings_.xmrNetworkAddress().valueChanged().connect(
      std::bind(&XmrManager::connect, manager.get(), ph::_1));
...
int XiboApp::run() {
  ...
  playerSettings_.statsEnabled().valueChanged().connect(
      [this](bool statsEnabled) {
        layoutManager_->statsEnabled(statsEnabled);
      });

  xmdsManager_ = std::make_unique<XmdsRequestSender>(
      cmsSettings_.address(), cmsSettings_.key(), cmsSettings_.displayId());
  ...
  Log::info("Current layout id: {}", scheduler_->currentLayoutId());

  scheduler_->layoutUpdated().connect([this]() {
    collectionInterval_->setCurrentLayoutId(scheduler_->currentLayoutId());
  });

  scheduler_->reloadSchedule(
      LayoutSchedule::fromFile(AppConfig::schedulePath()));
...
Uri XiboApp::localAddress() { return g_app->webserver_->address(); }

std::shared_ptr<ApplicationWindowGtk> XiboApp::createMainWindow() {
  std::shared_ptr<ApplicationWindowGtk> window = ApplicationWindowGtk::create(
      playerSettings_.size().width(), playerSettings_.size().height(),
      playerSettings_.position().x(), playerSettings_.position().y());

  playerSettings_.size().valueChanged().connect(
      [window](int width, int height) { window->setSize(width, height); });
  playerSettings_.position().valueChanged().connect(
      [window](int x, int y) { window->move(x, y); });

  window->statusScreenShown().connect([this, window]() {
    CHECK_UI_THREAD();
    StatusInfo info{collectGeneralInfo(), collectionInterval_->status(),
                    scheduler_->status(), xmrManager_->status()};

    window->updateStatusScreen(info, fileCache_->invalidFiles());
  });

  window->exitWithoutRestartRequested().connect(
      []() { System::terminateProccess(System::parentProcessId()); });

  return window;
}

std::unique_ptr<LayoutsManager> XiboApp::createLayoutManager() {
  auto manager = std::make_unique<LayoutsManager>(
      *scheduler_, *statsRecorder_, *fileCache_,
      playerSettings_.statsEnabled());
...
void XiboApp::checkResourceDirectory() {
  for (auto &&file : fileCache_->cachedFiles()) {
    if (!fileCache_->valid(file))
      continue;

    auto fullPath = cmsSettings_.resourcesPath() / file;
    if (!FileSystem::exists(fullPath) ||
        !fileCache_->cached(file, Md5Hash::fromFile(fullPath))) {
...
std::unique_ptr<CollectionInterval>
XiboApp::createCollectionInterval(XmdsRequestSender &xmdsManager) {
  auto interval = std::make_unique<CollectionInterval>(
      xmdsManager, *statsRecorder_, *fileCache_, cmsSettings_.resourcesPath(),
      playerSettings_.displayName());
  ...
  interval->collectionFinished().connect(
      std::bind(&XiboApp::onCollectionFinished, this, ph::_1));
  interval->scheduleAvailable().connect(
      std::bind(&Scheduler::reloadSchedule, scheduler_.get(), ph::_1));
  interval->filesDownloaded().connect(
      std::bind(&Scheduler::reloadQueue, scheduler_.get()));
...
std::unique_ptr<ScreenShotInterval>
XiboApp::createScreenshotInterval(XmdsRequestSender &xmdsManager,
                                  Xibo::Window &window) {
