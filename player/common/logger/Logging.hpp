...
create(const std::vector<spdlog::sink_ptr> &sinks);
...
template <typename T> void trace(const T &arg) {
...
template <typename T> void debug(const T &arg) {
...
template <typename T> void info(const T &arg) {
...
template <typename T> void error(const T &arg) {
