...
template <typename... Args> class Field {
...
public:
  ...
  void setValue(const Args &...values) { setValue(std::make_tuple(values...)); }
  ...
  const std::tuple<Args...> &values() const { return values_; }

  template <std::size_t I> const auto &value() const {
    return std::get<I>(this->values_);
  }

  template <std::size_t I> auto &value() { return std::get<I>(this->values_); }

  static constexpr std::size_t size() { return sizeof...(Args); }

  ValueChanged &valueChanged() { return *valueChanged_; }
...
template <typename T> class Field<T> {
  using ValueChanged = boost::signals2::signal<void(const T &)>;

public:
  explicit Field() : Field(T{}) {}
  explicit Field(const T &defaultValue)
      : value_{defaultValue}, valueChanged_(std::make_shared<ValueChanged>()) {}
  explicit Field(T &&defaultValue)
