...
Dbus::Error::Error(DBusError error)
    : PlayerRuntimeError("DBus", ""), error_(error) {}

// TODO managed objects

const char *Dbus::Error::what() const noexcept { return error_.name; }
...
DBusMessage *Dbus::createInhibitCall(const char *pid, const char *reason) {
  ...
  bool result =
      dbus_message_append_args(message, DBUS_TYPE_STRING, &pid,
...
uint32_t Dbus::sendAndWaitForResponse(DBusConnection *connection,
                                      DBusMessage *message) {
  uint32_t result = 0;

  auto reply = dbus_connection_send_with_reply_and_block(
      connection, message, DBUS_TIMEOUT_USE_DEFAULT, &error_);
  if (!reply)
    throw Dbus::Error{error_};

  bool success = dbus_message_get_args(reply, &error_, DBUS_TYPE_UINT32,
