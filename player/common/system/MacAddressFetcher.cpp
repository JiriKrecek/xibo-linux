...
ifreq MacAddressFetcher::findInterface(SocketDescriptor socket,
                                       ifconf &interfaceConfig) {
  const std::size_t configSize =
      static_cast<std::size_t>(interfaceConfig.ifc_len) / sizeof(ifreq);
...
std::string MacAddressFetcher::retrieveMacAddress(SocketDescriptor socket,
                                                  ifreq &interfaceRequest) {
  if (!ioctl(socket, SIOCGIFHWADDR, &interfaceRequest)) {
    char macAddress[MacAddressBuffer] = {0};
    unsigned char *mac =
        reinterpret_cast<unsigned char *>(interfaceRequest.ifr_hwaddr.sa_data);

    std::sprintf(macAddress, "%.2x:%.2x:%.2x:%.2x:%.2x:%.2x", mac[0], mac[1],
                 mac[2], mac[3], mac[4], mac[5]);

    return macAddress;
  }

  throw MacAddressFetcher::Error{"MAC",
                                 "MAC Address was not retrived from request"};
}

InterfaceFlags MacAddressFetcher::retrieveFlags(SocketDescriptor socket,
                                                ifreq &interfaceRequest) {
  ...
  throw MacAddressFetcher::Error{"MAC",
                                 "Flags were not retrived from frequest"};
}
