...
TEST(UriManualInit, SchemeHostAndPath)
{
    ...
    ASSERT_TRUE(uri.optionalAuthority()->host() == (HostTest{"test.com", Uri::Host::Type::DNS}));
...
TEST(UriManualInit, SchemeAuthorityAndPath)
{
    ...
    ASSERT_TRUE(static_cast<std::string>(uri.scheme()) == "http");
    ASSERT_TRUE(uri.path() == "/target");
    ASSERT_TRUE(uri.authority().host() == (HostTest{"test.com", Uri::Host::Type::DNS}));
    ...
    ASSERT_TRUE(static_cast<std::string>(uri.authority().userInfo()) == std::string{"username:password"});
}
