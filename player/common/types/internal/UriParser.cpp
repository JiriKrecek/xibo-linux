...
std::smatch UriParser::parseInternal(const std::string &rawUri) {
  std::smatch uriMatch;
  auto valid = std::regex_match(rawUri, uriMatch, UriRegex) &&
               uriMatch.size() > UriPartsCount;
  if (valid) {
    return uriMatch;
  }
...
boost::optional<Uri::Authority>
UriParser::authorityFrom(const std::smatch &results) {
...
boost::optional<Uri::UserInfo>
UriParser::userInfoFrom(const std::smatch &results) {
