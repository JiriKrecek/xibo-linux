...
bool FileSystem::exists(const FilePath &path) { return fs::exists(path); }

bool FileSystem::empty(const FilePath &path) { return fs::is_empty(path); }

bool FileSystem::remove(const FilePath &path) { return fs::remove(path); }
...
FilePath FileSystem::currentPath() { return fs::current_path(); }
