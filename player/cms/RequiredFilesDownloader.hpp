...
public:
  ...
  boost::future<DownloadResults>
  download(const RequiredFilesSet<RequiredFileType> &files) {
...
private:
  ...
  template <typename RequiredFileType>
  DownloadResults
  downloadAll(const RequiredFilesSet<RequiredFileType> &requiredFiles) {
  ...
  bool onResourceFileDownloaded(const ResponseContentResult &result,
