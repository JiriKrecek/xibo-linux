...
#include "cms/RequiredFilesDownloader.hpp"
...
CollectionInterval::CollectionInterval(XmdsRequestSender &xmdsSender,
    ...
    : xmdsSender_{xmdsSender}, statsRecorder_{statsRecorder},
      fileCache_{fileCache}, resourceDirectory_{resourceDirectory},
      displayName_{displayName}, intervalTimer_{std::make_unique<Timer>()},
      collectInterval_{DefaultInterval}, running_{false}, status_{},
      currentLayoutId_{EmptyLayoutId} {
  assert(intervalTimer_);
}

bool CollectionInterval::running() const { return running_; }

void CollectionInterval::stop() { workerThread_.reset(); }
...
void CollectionInterval::collectNow() {
  ...
  workerThread_ = std::make_unique<JoinableThread>([=]() {
    ...
    auto registerDisplayResult =
        xmdsSender_
            .registerDisplay(AppConfig::codeVersion(),
                             AppConfig::releaseVersion(), displayName_)
            .get();
...
void CollectionInterval::sessionFinished(const PlayerError &error) {
  ...
  startTimer();
  Log::debug(
      "[CollectionInterval] Finished. Next collection will start in {} seconds",
...
void CollectionInterval::onDisplayRegistered(
    const ResponseResult<RegisterDisplay::Result> &registerDisplay) {
  ...
  MainLoop::pushToUiThread([this, result = std::move(result.playerSettings)]() {
...
PlayerError CollectionInterval::displayStatus(
    const RegisterDisplay::Result::Status &status) {
  ...
  case DisplayCode::Waiting:
    return {"CMS", status.message};
  default:
    return {"CMS", "Unknown error with RegisterDisplay"};
  }
}

void CollectionInterval::updateInterval(int collectInterval) {
  if (collectInterval_ != collectInterval) {
    Log::debug("[CollectionInterval] Interval updated to {} seconds",
               collectInterval);
...
CmsStatus CollectionInterval::status() const { return status_; }
...
void CollectionInterval::onRequiredFiles(
    const ResponseResult<RequiredFiles::Result> &requiredFiles) {
...
void CollectionInterval::updateMediaInventory(
    const RequiredFiles::Result &result) {
...
void CollectionInterval::onSchedule(
    const ResponseResult<Schedule::Result> &schedule) {
...
void CollectionInterval::submitStats() {
  try {
    ...
    if (recordsCount > 0) {
      ...
      Log::debug("[CollectionInterval] Total records: {} Records to send {}",
                 recordsCount, RecordsToSend);
...
void CollectionInterval::onSubmitted(
    std::string_view requestName, const ResponseResult<Result> &submitResult) {
