...
using SignalSettingsUpdated =
    boost::signals2::signal<void(const PlayerSettings &)>;
using SignalScheduleAvailable = boost::signals2::signal<void(LayoutSchedule)>;
using SignalCollectionFinished =
    boost::signals2::signal<void(const PlayerError &)>;
...
public:
  CollectionInterval(XmdsRequestSender &xmdsSender,
                     Stats::Recorder &statsRecorder, FileCache &fileCache,
...
private:
  ...
  void
  onRequiredFiles(const ResponseResult<RequiredFiles::Result> &requiredFiles);
  ...
  void onSubmitted(std::string_view requestName,
                   const ResponseResult<Result> &submitResult);
