...
using SignalScheduleUpdated =
    boost::signals2::signal<void(const LayoutSchedule &)>;
...
private:
  ...
  void fillSchedulerStatus(SchedulerStatus &status,
                           const LayoutsList &layouts) const;
  void addDefaultToStatus(SchedulerStatus &status,
