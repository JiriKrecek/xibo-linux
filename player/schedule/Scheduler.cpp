...
Scheduler::Scheduler(const FileCache &fileCache)
    : fileCache_{fileCache}, schedule_{} {}
...
LayoutId Scheduler::nextLayout() const { return regularQueue_.next(); }

LayoutId Scheduler::currentLayoutId() const { return regularQueue_.current(); }

OverlaysIds Scheduler::overlayLayouts() const {
  return overlayQueue_.overlays();
}

SignalScheduleUpdated &Scheduler::scheduleUpdated() { return scheduleUpdated_; }

SignalLayoutsUpdated &Scheduler::overlaysUpdated() { return overlaysUpdated_; }
...
DateTime Scheduler::closestLayoutDt() {
  ...
  for (auto &&layout : schedule_->regularLayouts) {
    if (now < layout.startDT && layout.startDT < closestDt)
      closestDt = layout.startDT;
    if (now < layout.endDT && layout.endDT < closestDt)
      closestDt = layout.endDT;
  }
...
void Scheduler::restartTimer() {
  ...
  ulong duration = (dt - DateTime::now()).total_seconds();

  if (dt.valid() && duration > 0) {
    Log::trace("[Scheduler] Timer restarted: {}", duration);

    timer_.startOnce(std::chrono::seconds(duration),
                     [this] { this->reloadQueue(); });
  }
}

SignalLayoutsUpdated &Scheduler::layoutUpdated() { return layoutUpdated_; }
...
boost::optional<ScheduledLayout> Scheduler::layoutById(int id) const {
...

  {
    ...
    auto it = std::find_if(
        regularLayouts.begin(), regularLayouts.end(),
        [id](const ScheduledLayout &other) { return other.id == id; });

    if (it != regularLayouts.end())
      return *it;
  }
  {
    auto &&overlayLayouts = schedule_->overlayLayouts;
    auto it = std::find_if(
        overlayLayouts.begin(), overlayLayouts.end(),
        [id](const ScheduledLayout &other) { return other.id == id; });

    if (it != overlayLayouts.end())
      return *it;
  }
...
void Scheduler::fillSchedulerStatus(SchedulerStatus &status,
                                    const LayoutsList &layouts) const {
...
void Scheduler::addDefaultToStatus(SchedulerStatus &status,
                                   const DefaultScheduledLayout &layout) const {
  if (layoutValid(layout)) {
    status.validLayouts.emplace_back(layout.id);
  } else {
    status.invalidLayouts.emplace_back(layout.id);
