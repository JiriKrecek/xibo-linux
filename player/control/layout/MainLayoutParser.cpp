...
MainLayoutParser::Error::Error(const std::string &domain, int layoutId,
                               const std::string &reason)
    : PlayerRuntimeError{domain,
                         "Layout " + std::to_string(layoutId) +
                             " is invalid or missing. Reason: " + reason} {}

MainLayoutParser::MainLayoutParser(bool globalStatsEnabled)
    : globalStatsEnabled_{globalStatsEnabled} {}
...
std::unique_ptr<Xibo::MainLayout> MainLayoutParser::parseBy(int layoutId) {
  try {
    ...
    throw MainLayoutParser::Error{"LayoutParser - " + e.domain(), layoutId,
                                  e.message()};
...
std::unique_ptr<Xibo::MainLayout>
MainLayoutParser::layoutFrom(const XmlNode &node) {
...
MainLayoutOptions MainLayoutParser::optionsFrom(const XmlNode &node) {
  ...
  auto statsEnabled = globalStatsEnabled_ &&
                      node.get<bool>(XlfResources::MainLayout::StatsEnabled,
                                     DefaultStatsEnabled);

  auto backgroundUri = backgroundUriFrom(node);
  auto backgroundColor = backgroundColorFrom(node);

  return MainLayoutOptions{layoutId_,    width,         height,
                           statsEnabled, backgroundUri, backgroundColor};
}

boost::optional<Uri> MainLayoutParser::backgroundUriFrom(const XmlNode &node) {
  auto uri =
      node.get_optional<std::string>(XlfResources::MainLayout::BackgroundPath);
  if (uri) {
...
std::shared_ptr<Xibo::Image>
MainLayoutParser::createBackground(const MainLayoutOptions &options) {
  if (options.backgroundUri)
    return ImageWidgetFactory::create(options.backgroundUri.value(),
                                      options.width, options.height,
                                      Xibo::Image::PreserveRatio::False);
  else
    return ImageWidgetFactory::create(options.backgroundColor, options.width,
                                      options.height);
}

void MainLayoutParser::addRegions(Xibo::MainLayout &layout,
                                  const XmlNode &layoutNode) {
  for (auto [nodeName, node] : layoutNode) {
    if (nodeName != XlfResources::RegionNode)
      continue;

    RegionParser parser{globalStatsEnabled_};
    auto position = parser.positionFrom(node);
    layout.addRegion(parser.regionFrom(node), position.left, position.top,
                     position.zorder);
  }
}
