...
LayoutsManager::LayoutsManager(Scheduler &scheduler,
    ...
    : scheduler_(scheduler), statsRecorder_(statsRecorder),
      fileCache_(fileCache), statsEnabled_{statsEnabled} {
  scheduler_.layoutUpdated().connect(
      std::bind(&LayoutsManager::fetchMainLayout, this));
  scheduler_.overlaysUpdated().connect(
      std::bind(&LayoutsManager::fetchOverlays, this));
}
...
OverlaysLoaded &LayoutsManager::overlaysFetched() { return overlaysFetched_; }
...
void LayoutsManager::statsEnabled(bool enable) { statsEnabled_ = enable; }
...
std::unique_ptr<Xibo::MainLayout> LayoutsManager::createLayout(int layoutId) {
  try {
    ...
    layout->statReady().connect(
        [this, layoutId, scheduleId](const Stats::PlayingTime &interval) {
          try {
            statsRecorder_.addLayoutRecord(
                Stats::LayoutRecord::create(scheduleId, layoutId, interval));
          } catch (const std::exception &e) {
            Log::error("[LayoutsManager] {}", e.what());
          }
        });
    layout->mediaStatsReady().connect(
        [this, layoutId, scheduleId](const MediaPlayingTime &intervals) {
          try {
            Stats::MediaRecords records;
            for (auto &&[mediaId, interval] : intervals) {
              records.add(Stats::MediaRecord::create(scheduleId, layoutId,
                                                     mediaId, interval));
            }
    ...
    Log::error("[LayoutsManager] {}", e.what());
    Log::info(
        "[LayoutsManager] Check resource folder to find out what happened");
  }
