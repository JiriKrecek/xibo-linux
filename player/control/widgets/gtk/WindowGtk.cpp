...
WindowGtk::WindowGtk()
    : SingleContainerGtk{handler_}, cursorVisible_(true), fullscreen_(false) {
  handler_.signal_realize().connect(
      std::bind(&WindowGtk::onWindowRealized, this));
  ...
  handler_.signal_window_state_event().connect([this](
                                                   GdkEventWindowState *event) {
    if (event->changed_mask & GDK_WINDOW_STATE_FULLSCREEN) {
      bool fullscreen = event->new_window_state & GDK_WINDOW_STATE_FULLSCREEN;
      if (!fullscreen) {
        // setSize right after unfullscreen() only change virtual size of the
        // window but not physical because unfullscreen() is async operation and
        // only after this execution we can restore previous physical window
        // dimensions.
...
void WindowGtk::removeFromHandler(
    const std::shared_ptr<Xibo::Widget> & /*child*/) {
  handler_.remove();
}

void WindowGtk::setSize(int width, int height) { resizeWindow(width, height); }
...
void WindowGtk::move(int x, int y) { handler_.move(x, y); }

void WindowGtk::disableWindowResize() { handler_.set_resizable(false); }

void WindowGtk::disableWindowDecoration() { handler_.set_decorated(false); }

SignalKeyPressed &WindowGtk::keyPressed() { return keyPressed_; }

void WindowGtk::fullscreen() {
  auto monitorGeometry = currentMonitorGeometry();
  if (!monitorGeometry.has_zero_area()) {
    // Fullscreen means that window will be shown on monitor dimensions withour
    // borders but children are not automatically resized during this operation
    // so we need to force them to update their sizes. Also, we need to save
    // previous window dimensions to restore them later
...
bool WindowGtk::isFullscreen() const { return fullscreen_; }
...
void WindowGtk::setBackgroundColor(const Color &color) {
  ...
  if (cssProvider && styleContext && screen &&
...
Gtk::Window &WindowGtk::handler() { return handler_; }
