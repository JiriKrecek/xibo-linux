...
int WebViewParser::durationFrom(const XmlNode &node) {
  ...
  return parseDuration(FilePath(uriFrom(node).path())).value_or(baseDuration);
}
...
boost::optional<int> WebViewParser::parseDuration(const FilePath &path) {
  ...
  return matchedGroups.size() > 1
             ? std::stoi(matchedGroups[DurationGroup].str())
             : boost::optional<int>{};
}
...
Uri WebViewParser::uriFrom(const XmlNode &node) {
  ...
  if (mode != NativeModeid) {
    std::string fileName{std::to_string(idFrom(node)) +
                         DefaultWebviewExtension};
...
std::unique_ptr<Xibo::Media>
WebViewParser::createMedia(const MediaOptions &options, const XmlNode &node,
                           int width, int height) {
  auto transparency =
      node.get<bool>(XlfResources::WebView::Transparency, DefaultTransparency);
