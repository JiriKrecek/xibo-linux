...
GstMediaPlayer::GstMediaPlayer()
  ...
  if (!playbin_)
    throw Error{"GstMediaPlayer",
                "Unable to create player: playbin is missing."};
  if (!videoSink_)
    throw Error{"GstMediaPlayer",
                "Unable to create player: gtkglsink is missing."};
  if (!glSinkBin_)
    throw Error{"GstMediaPlayer",
                "Unable to create player: glsinkbin is missing."};
  ...
  g_object_get(videoSink_, "widget", &videoSinkWidget,
               nullptr); // transfer ownership here, ref_count == 2
  if (videoSinkWidget) {
    // take ownership and unref widget from sink
    outputWindow_ =
        std::make_shared<OutputWindowGtk>(Glib::wrap(videoSinkWidget));
    g_object_unref(videoSinkWidget);
  }

  auto bus = gst_element_get_bus(playbin_);
  busWatchId_ = gst_bus_add_watch(
      bus, static_cast<GstBusFunc>(&GstMediaPlayer::busMessageWatch), this);
...
void GstMediaPlayer::setVolume(int volume) {
  check(volume);
  g_object_set(playbin_, "volume", volume / static_cast<double>(MaxVolume),
               nullptr);
}

void GstMediaPlayer::setAspectRatio(MediaGeometry::ScaleType scaleType) {
  bool aspectRatio =
      scaleType == MediaGeometry::ScaleType::Scaled ? true : false;
  g_object_set(playbin_, "force-aspect-ratio", aspectRatio, nullptr);
}

void GstMediaPlayer::check(int volume) {
  if (volume < MinVolume || volume > MaxVolume)
    throw Error{"GstMediaPlayer", "Volume should be in [0-100] range"};
}
...
void GstMediaPlayer::stop() { gst_element_set_state(playbin_, GST_STATE_NULL); }
...
void GstMediaPlayer::setOutputWindow(
    const std::shared_ptr<Xibo::OutputWindow> &outputWindow) {
  assert(outputWindow);

  outputWindow_ = outputWindow;
}

const std::shared_ptr<Xibo::OutputWindow> &
GstMediaPlayer::outputWindow() const {
  return outputWindow_;
}

// we don't need to unref bus here
gboolean GstMediaPlayer::busMessageWatch(GstBus * /*bus*/, GstMessage *msg,
                                         gpointer data) {
  switch (msg->type) {
  ...
  case GST_MESSAGE_ERROR: {
    ...
    Log::error("[GstMediaPlayer] Error from element {}: {}", msg->src->name,
               err->message);
    if (debug_info) {
