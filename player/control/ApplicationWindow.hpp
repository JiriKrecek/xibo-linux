...
template <typename Window> class ApplicationWindow : public Window {
...
public:
  static std::unique_ptr<ApplicationWindow<Window>>
  create(int width, int height, int x, int y) {
    namespace ph = std::placeholders;

    std::unique_ptr<ApplicationWindow<Window>> window(
        new ApplicationWindow<Window>());

    window->setSize(width, height);
    window->move(x, y);
    window->setMainContainer(
        OverlayContainerFactory::create(window->width(), window->height()));
    window->disableWindowDecoration();
    window->setCursorVisible(false);
    window->setBackgroundColor(Color::fromString("#000000"));
    window->keyPressed().connect(
        std::bind(&ApplicationWindow::onKeyPressed, window.get(), ph::_1));
  ...
  StatusScreenShown &statusScreenShown() { return statusScreenShown_; }
  ...
  void setSize(int width, int height) override {
    ...
    statusScreen_->setSize(
        static_cast<int>(this->width() * StatusScreenScaleX),
        static_cast<int>(this->height() * StatusScreenScaleY));
  }

private:
  ApplicationWindow()
      : statusScreen_(StatusScreenFactory::create(static_cast<Window &>(*this),
                                                  MinStatusScreenWidth,
                                                  MinStatusScreenHeight)) {
    assert(statusScreen_);
  }

  std::pair<int, int> adjustSize(int width, int height) {
    if (width < 0 || height < 0) {
      Log::error("[ApplicationWindow] Negative size is not allowed, falling "
  ...
  void scaleLayout(const std::shared_ptr<Xibo::Widget> &layout) {
    auto scaleFactor = this->calcScaleFactor(layout->width(), layout->height(),
  ...
  std::shared_ptr<Xibo::Image> createSplashScreen() {
    return ImageWidgetFactory::create(
        Uri::fromFile(AppConfig::splashScreenPath()), this->width(),
        this->height(), Xibo::Image::PreserveRatio::False);
  }
